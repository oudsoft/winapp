...
const util = require('./utility.js')(log);
			...
			if (hrPatientFiles.length > 0) {
				for (let i=0; i < hrPatientFiles.length; i++) {
					if (!hrPatientFiles[i].instanceId) {
						...
						let command = util.formatStr('convert -verbose -density 150 -trim %s/%s', jpgDir, jpgFileName);
...
var doTransferDicomZipFile = function(studyID, outputFilename){
	return new Promise(async function(resolve, reject) {
		...
		var command = 'curl --list-only --user sasurean:drinking@min -T ' + dest + ' ftp://150.95.26.106/Radconnext/public/img/usr/zip/  -v';
