...
const doRemoveOldImages = function(oldImages, convertItems){
	return new Promise(function(resolve, reject) {
		//let resultImages = [];
		let promiseList = new Promise(async function(resolve2, reject2){
...
const doConvertJPG2DCM = function(hrPatientFiles, studyTags, oldImages) {
	return new Promise(function(resolve, reject) {
		let convertItems = [];
		let promiseList = new Promise(async function(resolve2, reject2){
	...
	return new Promise(async(resolve, reject)=>{
		...
		if (tmps.length == 2) {
			...
			if ((fileExt === 'zip') || (fileExt === 'rar')){
				...
				let command = undefined;
				if (process.env.OS_NAME == 'LINUX') {
					command = util.formatStr('mv %s%s %s/%s', process.env.LOCAL_ATTACH_DIR, oldFileName, process.env.LOCAL_ATTACH_DIR, newFilename);
				} else {
					command = util.formatStr('ren %s%s %s/%s', process.env.LOCAL_ATTACH_DIR, oldFileName, process.env.LOCAL_ATTACH_DIR, newFilename);
				}
