	...
	return new Promise(function(resolve, reject) {
		...
		let promiseList = new Promise(async function(resolve2, reject2){
			for (let i=0; i < oldImages.length; i++) {
				...
				if (item.instanceId) {
					let command = util.formatStr('curl -X DELETE --user demo:demo http://' + process.env.ORTHANC_IP + ':8042/instances/%s', item.instanceId);
				...
				for (let i=0; i < hrPatientFiles.length; i++) {
					//if (!hrPatientFiles[i].instanceId) {
						...
						if (supportConvertTyps.includes(hrPictureType)) {
							...
							if ((hrPictureType.toLowerCase() === 'png') || (hrPictureType.toLowerCase() === 'bmp')) {
								if (process.env.OS_NAME === 'LINUX') {
									let stdout = await util.runcommand('mogrify -format jpg ' + jpgDir + '/' + jpgFileName);
									stdout = await util.runcommand('rm ' + jpgDir + '/' + jpgFileName);
									jpgFileName = hrPatientFiles[i].code + '.jpg';
									stdout = await util.runcommand('chmod 0664 ' + jpgDir + '/' + jpgFileName);
								} else {
									let stdout = await util.runcommand('mogrify -format jpg ' + jpgDir + '/' + jpgFileName);
									stdout = await util.runcommand('del ' + jpgDir + '/' + jpgFileName);
									jpgFileName = hrPatientFiles[i].code + '.jpg';									
								}
							}
							...
							*/
							...
							command = util.formatStr('curl -X POST --user demo:demo http://' + process.env.ORTHANC_IP + ':8042/instances --data-binary @%s/%s', dcmDir, dcmFileName);
							log.info('command=> ' + command);
